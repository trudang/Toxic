...
labels = ['SD < toxicKernel - 5%', 'SD == toxicKernel +- 5%', 'SD > toxicKernel + 5%']

# Import data of all ratings
workbook_input = xlrd.open_workbook('tweet_comparison_data/tweet10_comparison_total.xls')
...
toxicKernel = [sheet_input.cell_value(row, raters['toxicKernel']) for row in xrange(1, sheet_input.nrows)]
...
def contigencyTable(x, y, workbook):
	...
	for var in [new_x, new_y]:
		for i in range(len(var)):
			# print var[i], toxicKernel[i] - .05*toxicKernel[i], toxicKernel[i] + .05*toxicKernel[i]
			if toxicKernel[i] - .05*toxicKernel[i] <= var[i] <= toxicKernel[i] + .05*toxicKernel[i]:
				# print 'equal'
				var[i] = 0
			elif var[i] < toxicKernel[i] - .05*toxicKernel[i]:
				# print 'smaller'
				var[i] = -1
			elif var[i] > toxicKernel[i] + .05*toxicKernel[i]:
				# print 'larger'
	...
	for i in range(len(toxicKernel)):
...
def contigencyTables_to_xls(combs):
	...
	workbook.save("tweet10_comparison_contigency_toxicKernel.xls")

raters = [tru, mike, nick, toxicKernel]
