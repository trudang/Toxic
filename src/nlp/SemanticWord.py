...
class SemanticWord(object):
	...
	def __init__(self,word,part_of_speech,lookuptable,kernel):
		...
		self.db = lookuptable
		self.kernel = kernel
		self.in_kernel = True if self.word in self.kernel and self.word not in bad_words else False
	...
	def lookup(self,other):
		...
		if query not in self.db:
			...
			if transpose_query in self.db:
				...
				#TEST
				print "Word-pair: %s -VS- %s" %(self.word,other.word)
				print "Average distance between the 2 words:", np.sum(distance)/float(divisor) if distance != [] else None
				#DONE
	...
	def __sub__(self,other):
		if self.synset and other.synset and self.part_of_speech == other.part_of_speech: 
			...
			#TEST
			print "Word-pair: %s -VS- %s" %(self.word, other.word)
			print "Not same part_of_speech ==> Distance: None"
			#DONE
			return None
