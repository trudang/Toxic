...
txt_name = 'db-text.txt'
with open(txt_name) as f:
  strings = [tweet.strip() for tweet in f.readlines()]

all_words = [token.word for string in strings for token in SemanticString(string, {}).tokens]

# print len(all_words)

words = [word for word in all_words if SemanticWord(word,'NN',{}).synset != None]
